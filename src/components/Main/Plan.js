  ...
  return (
    <Content>
      ...
      <div className=" h-10 w-full flex items-center justify-center">
        {selectedPlan !== null || (
          <p className="text-gray-400 text-sm">Make sure a plan is selected!</p>
        )}
      </div>
...
const PlanButton = ({ plan, selectedPlan, onClick }) => {
  return (
    <button
      className={`border p-4 flex flex-col rounded-md w-full ${
        plan === selectedPlan ? "bg-veryLightBlue border border-marineBlue" : ""
      }`}
      onClick={onClick}
    >
      {plan.icon()}
      <span className="font-bold text-md">{plan.name}</span>
      <span className="text-gray-400">${plan.price}/yr</span>
      <span className="text-sm">{plan.freeMonthTerm} month free</span>
    </button>
  );
};

const ToggleBillingDuration = ({ onSetBillingDuration, billDuration }) => {
  const handleToggle = (e) => {
    e.preventDefault();
    onSetBillingDuration((duration) =>
      duration === "monthly" ? "yearly" : "monthly"
    );
  };

  return (
    <div className="bg-slate-100 p-4 flex justify-center items-center space-x-5 rounded-md">
      <span
        className={
          billDuration === "monthly" ? "text-marineBlue" : "text-slate-400"
        }
      >
        Monthly
      </span>
      <ToggleButton handleToggle={handleToggle} billDuration={billDuration} />
      <span
        className={
          billDuration === "yearly" ? "text-marineBlue" : "text-slate-400"
        }
      >
        Yearly
      </span>
